...
class AsyncClient {
  ...
  async rpcCall(methodName, params = "", config = {}, timeout = this.#timeout) {
    ...
    const { header: headerWithPayloadInfoAndId, serializedPayload } =
      mergedConfig.serializePayload(headerWithId, params);
  ...
  async #enqueueRequest(id, originalHeader, serializedPayload) {
    ...
    for (let index = 0; index < totalChunks; ) {
      ...
      const header = this.#createHeaderForChunk(
        originalHeader,
        index,
        totalChunks
      );
  ...
  async #prepareAndEnqueueFollowup(id, header, response) {
    ...
    const newHeader = this.#addResponseIdToHeader(
  ...
  async #dispatchRequest(header, serializedPayload = "") {
    ...
    this.#log("Dispatching Request :", {
      header: headerWithSessionId,
      serializedPayload,
    });
    const response = await this.#serverConnectionStrategy.request(
      headerWithSessionId,
      serializedPayload
    );
...
class TcHmiSymbolRequestStrategy extends ServerConnectionStrategy {
  ...
  async request(header, serializedPayload) {
    const requestPayload = this.#prepareRequestParams(
      header,
      serializedPayload
    );
  ...
  #processServerResponse(serverResponse) {
    const response = serverResponse.Header
      ? Headers.toJson(serverResponse.Header)
      : {};
